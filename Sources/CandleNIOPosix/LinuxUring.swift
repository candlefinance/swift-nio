...
final internal class URing {
    internal static let POLLIN: CUnsignedInt = numericCast(CandleCNIOLinux.POLLIN)
    internal static let POLLOUT: CUnsignedInt = numericCast(CandleCNIOLinux.POLLOUT)
    internal static let POLLERR: CUnsignedInt = numericCast(CandleCNIOLinux.POLLERR)
    internal static let POLLRDHUP: CUnsignedInt = CNIOLinux_POLLRDHUP()  // numericCast(CandleCNIOLinux.POLLRDHUP)
    internal static let POLLHUP: CUnsignedInt = numericCast(CandleCNIOLinux.POLLHUP)
    ...
    internal func io_uring_queue_init() throws {
        if CandleCNIOLinux.io_uring_queue_init(ringEntries, &ring, 0) != 0 {
    ...
    internal func io_uring_queue_exit() {
        ...
        CandleCNIOLinux.io_uring_queue_exit(&ring)
    }
    ...
    func withSQE<R>(_ body: (UnsafeMutablePointer<io_uring_sqe>?) throws -> R) rethrows -> R {
        ...
        //
        while true {
            if let sqe = CandleCNIOLinux.io_uring_get_sqe(&ring) {
                return try body(sqe)
            }
            self.io_uring_flush()
        }
    ...
    internal func io_uring_flush() {  // When using SQPOLL this is basically a NOP
        ...
        waitingSubmissions = CandleCNIOLinux.io_uring_sq_ready(&ring)

        loop: while waitingSubmissions > 0 {
            retval = CandleCNIOLinux.io_uring_submit(&ring)
            submissionCount += 1
    ...
    internal func io_uring_prep_poll_add(
        ...
        self.withSQE { sqe in
            CandleCNIOLinux.io_uring_prep_poll_add(sqe, fileDescriptor, pollMask)
            // must be done after prep_poll_add, otherwise zeroed out.
            CandleCNIOLinux.io_uring_sqe_set_data(sqe, bitpatternAsPointer)
    ...
    internal func io_uring_prep_poll_remove(
        ...
        self.withSQE { sqe in
            CandleCNIOLinux.io_uring_prep_poll_remove(sqe, .init(userData: bitPattern))
            // must be done after prep_poll_add, otherwise zeroed out.
            CandleCNIOLinux.io_uring_sqe_set_data(sqe, .init(userData: userbitPattern))
    ...
    internal func io_uring_poll_update(
        ...
        self.withSQE { sqe in
            ...
            CandleCNIOLinux.io_uring_prep_poll_update(
                sqe,
    ...
    internal func io_uring_peek_batch_cqe(
        ...
        var eventCount = 0
        var currentCqeCount = CandleCNIOLinux.io_uring_peek_batch_cqe(&ring, cqes, cqeMaxCount)
    ...
    internal func io_uring_wait_cqe(
        ...
        let error = CandleCNIOLinux.io_uring_wait_cqe(&ring, cqes)
    ...
    internal func io_uring_wait_cqe_timeout(
        ...
        let error = CandleCNIOLinux.io_uring_wait_cqe_timeout(&ring, cqes, &ts)
